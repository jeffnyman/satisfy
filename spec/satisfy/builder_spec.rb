RSpec.describe Satisfy::Builder do
  let(:builder) { Satisfy::Builder.build(test_spec) }
  ...
  context 'representative test spec file' do
    let(:test_spec) { File.expand_path('../../specs/scenario.spec', File.dirname(__FILE__)) }
    let(:scenarios) { feature.scenarios }
    let(:steps) { feature.scenarios.first.steps }
  ...
  context 'test spec file with scenario outline' do
    let(:test_spec) { File.expand_path('../../specs/scenario_outline.feature', File.dirname(__FILE__)) }
  ...
  context 'test spec with a doc string' do
    let(:test_spec) { File.expand_path('../../specs/scenario_with_docstring.story', File.dirname(__FILE__)) }
  ...
  context 'test spec file with table step argument' do
    let(:test_spec) { File.expand_path('../../specs/scenario_with_table.spec', File.dirname(__FILE__)) }
    let(:scenarios) { feature.scenarios }
    let(:steps) { feature.scenarios.first.steps }

    it 'has a step with a table' do
      table = steps[0].step_args.find { |a| a.instance_of?(Satisfy::Table) }
  ...
  context 'test spec file with scenario outline and data table' do
    let(:test_spec) { File.expand_path('../../specs/scenario_outline_with_table.spec', File.dirname(__FILE__)) }
    ...
    it 'will replace placeholders in steps' do
      ...
      table = feature.scenarios[0].steps[0].step_args.find { |arg| arg.instance_of? (Satisfy::Table) }
      expect(table.hashes[0]['hit points']).to eq '100'
      expect(table.hashes[0]['agility']).to eq '10'

      table = feature.scenarios[1].steps[0].step_args.find { |arg| arg.instance_of? (Satisfy::Table) }
      expect(table.hashes[0]['hit points']).to eq '100'
  ...
  context 'test spec with a scenario outline and doc string' do
    let(:test_spec) { File.expand_path('../../specs/scenario_outline_with_docstring.story', File.dirname(__FILE__)) }
